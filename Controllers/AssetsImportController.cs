...
public class ImportController : ControllerBase
{
        ...
        for (int row = 2; row <= rowCount; row++) // ვივარაუდებთ, რომ პირველი ხაზი header-ია
        {
            try
            {
                ...
                var asset = new Asset
                {
                    ...
                    AssetStatusId = await ResolveLookupId("AssetStatus", worksheet.Cells[row, 11].GetValue<string>()) ?? 1,
                    ResponsiblePersonId = await ResolveLookupId("Employees", worksheet.Cells[row, 12].GetValue<string>()),  // ← აქ ResponsiblePersonId!
    ...
    private async Task<int?> ResolveLookupId(string tableName, string name)
    {
        ...
        return tableName switch
        {
            ...
            "Locations" => await Task.Run(() => _context.Locations
                .Include(l => l.Building)
                .AsEnumerable()
                .Where(l => $"{l.Building.Name} - {l.RoomNumber}".Trim() == name.Trim())
                .Select(l => (int?)l.Id)
                .FirstOrDefault()),

            "AssetStatus" => await _context.AssetStatus
                .Where(s => s.StatusName == name.Trim())
                .Select(s => (int?)s.Id)
                .FirstOrDefaultAsync(),
